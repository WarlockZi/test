...
class TelegramBot
{
    ...
    public function __construct(string $channel)
    {
        ...
        $this->TELEGRAM_VitexTestBot_TOKEN     = getenv('TELEGRAM_VitexTestBot_TOKEN');
    }

    private function setChanel($channel): void
    {
        if ($channel===TGChannel::callme->name) {
            $this->chat_id = '-100'.getenv('TELEGRAM_VITEX_CALLME_CHANNAL_ID');

        }elseif ($channel===TGChannel::question->name){
            $this->chat_id = '-100'.getenv('TELEGRAM_VITEX_SALES_CHANNAL_ID');
        }
