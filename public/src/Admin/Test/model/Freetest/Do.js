...
//   show first card
let showedCard = document
  .querySelector(" .question:first-child ")
  .getAttribute("data-id");
document.querySelector(`.question[data-id="${showedCard}"`).style.display =
  "flex";

document.querySelector(".content #finish-freetest").addEventListener(
  "click",
  function (event) {
    ...
    function colorView(obj) {
      return new Promise(function (resolve, reject) {
        ...
        textarea.forEach(function (elem, tindex, textarea) {
          // перебираем txtarea
          let text = elem.innerHTML;
          if (text) {
            let textareaId = elem.dataset.textarea,
              paginItem = document.querySelectorAll(
                'a[href="#question-' + textareaId + '"]',
              )[0],
        ...
        for (let i = 0; i < a.length; i++) {
          if (a[i].autocomplete.boxShadow != green) {
            // Если не красный, раскрасим в зеленый
