...
class Question extends Model {
  ...
  fullfill(target) {
    ...
    let sort =
      +$(el).find(this.sortSelector).innerText > 0
  ...
  getTest_id() {
    if (this.target)
      return this.target.closest(this.testSelector).dataset.testId;
    return null;
  }
...
export default function question(target) {
  return new Question(target);
}
