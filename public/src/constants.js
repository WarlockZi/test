"use strict";
export const d = "document";
export const qs = "querySelector";
export const qa = "querySelectorAll";
export const ael = "addEventListener";
export const rel = "removeEventListener";
export const st = "setTimeout";
export const it = "innerText";
export const checkboxSelector = "my-checkbox";
export const searchSelector = "searchable-select";
export const catItemSelector = ".item-wrap";
export const tableSelector = "custom-table";
export const quillSelector = "[data-quill]";
export const dataRelationSelector = "[data-relation]";
export const dataPivotSelector = "[data-pivot]";

export const cqs = document.querySelector.bind(document);
