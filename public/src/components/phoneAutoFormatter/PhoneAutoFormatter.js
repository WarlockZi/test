  ...
  if (formattedPhone) {
    ...
    if (throwExceptionWhenNoMatch) {
      throw (
        'Phone "' +
        rawPhone +
        "\" didn't match any of the patterns:" +
        "\n" +
        this.patterns.join(",\n")
      );
    } else {
 ...
 */
Freedom.PhoneFormatter.prototype.setValueAndHandleCursor = function (
  input,
  newValue,
) {
...
/** @private */
Freedom.PhoneFormatter.prototype.isPatternStringValid = function (
  patternString,
) {
...
/** @private */
Freedom.PhoneFormatter.prototype.throwInvalidPatternError = function (
  patternString,
) {
  var msg =
...
Freedom.PhoneFormatter.prototype.attachToSelector = function (selector) {
  if (!Freedom.SelectorWatcher) {
    const msg =
      "You need Freedom.SelectorWatcher library.\n" +
