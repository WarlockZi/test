...
/*!************************************************!*\
  !*** ./public/src/Adm_catalog/adm_category.js ***!
  \************************************************/
$(function () {
  function obj(action) {
    return {
  ...
  $('.category-update-btn').on('click', function () {
    ...
    var props = $('.properties select option:selected'); //      debugger;

    var prop = [];

    for (let val of props) {
      if (val.value) prop.push(val.value);
    }
  ...
  $('.properties.column').on('change', '.new-prop', function () {
    ...
    $(clone).find('option[value = ' + val + ']').remove(); //      debugger;
